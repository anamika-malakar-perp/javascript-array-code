...
//Negative index
console.log(fruits.at(-1)); //fruits[fruits.length-1]

//Length
// The length property automatically updates when we modify the array. To be precise, it is actually not the count of values in the array, but the greatest numeric index plus one.

let fruitsData = [];
fruitsData[123] = 'Apple';

console.log(fruitsData.length); // 124

// If we increase it manually, nothing interesting happens. But if we decrease it, the array is truncated. The process is irreversible, here’s the example

let truncateArr = [1, 2, 3, 4, 5];

truncateArr.length = 2; // truncate to 2 elements
console.log(truncateArr); // [1, 2]

truncateArr.length = 5;
console.log(truncateArr);

//new Array
let predefinedArr = new Array(2);

console.log('pre', predefinedArr);
